...
class WebMCP {
    constructor(options = {}) {
        ...
        // Storage keys for localStorage (persisted across page refreshes)
    ...
    /**
     * Check for stored connection info in localStorage and connect if found
     * @private
     */
    _checkStoredToken() {
        const storedConnectionInfo = localStorage.getItem(this.SESSION_STORAGE_KEY);

        if (storedConnectionInfo) {
            try {
                const connectionInfo = JSON.parse(storedConnectionInfo);
                if (connectionInfo.token && connectionInfo.server) {
                    console.log('Found stored connection info, attempting to reconnect');

                    // Restore connection properties directly from stored session token
                    this.currentServer = connectionInfo.server;
                    this.currentToken = connectionInfo.token;
                    this.currentChannel = `/${connectionInfo.channelHost || this._format(this.options.channelName || window.location.host)}`;
    ...
    _saveItemsToStorage() {
        try {
            ...
            localStorage.setItem(this.TOOLS_STORAGE_KEY, JSON.stringify(toolsData));
            ...
            localStorage.setItem(this.PROMPTS_STORAGE_KEY, JSON.stringify(promptsData));
            ...
            localStorage.setItem(this.RESOURCES_STORAGE_KEY, JSON.stringify(resourcesData));
    ...
    _loadStoredItems() {
        try {
            // Load tools
            const storedTools = localStorage.getItem(this.TOOLS_STORAGE_KEY);
            if (storedTools) {
            ...
            // Load resources
            const storedResources = localStorage.getItem(this.RESOURCES_STORAGE_KEY);
            if (storedResources) {
     ...
     */
    _clearStoredItems() {
        localStorage.removeItem(this.TOOLS_STORAGE_KEY);
        localStorage.removeItem(this.PROMPTS_STORAGE_KEY);
        localStorage.removeItem(this.RESOURCES_STORAGE_KEY);
    ...
    async connect(connectionToken) {
        ...
        try {
            ...
            // Store the connection info in localStorage for page navigations
            const connectionInfo = {
                token: connectionToken,
                server: this.currentServer,
                host: this._format(this.options.channelName || window.location.host)
            };

            // Check if we have connection data already in localStorage
            const storedConnectionInfo = localStorage.getItem(this.SESSION_STORAGE_KEY);
            ...
            if (storedConnectionInfo) {
                try {
                    ...
                    if (connectionInfo.server === this.currentServer &&
                        connectionInfo.host === this._format(this.options.channelName || window.location.host)) {
            ...
            if (!skipRegistration) {
                ...
                localStorage.setItem(this.SESSION_STORAGE_KEY, JSON.stringify(connectionInfo));
            }
    ...
    _processConnectionToken(encodedToken) {
        try {
            ...
            this.currentChannel = `/${this._format(this.options.channelName || window.location.host)}`;
    ...
    _registerWithServer(encodedToken) {
        ...
        return new Promise((resolve, reject) => {
            ...
            regSocket.addEventListener('open', (event) => {
                ...
                const jsonStr = atob(encodedToken);
                const connectionData = JSON.parse(jsonStr);
                connectionData.host = this._format(this.options.channelName || window.location.host);
...
// Expose as global for browser script-tag usage
