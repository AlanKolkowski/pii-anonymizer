...
describe('entity-sources config', () => {
  ...
  // 'lexicon' (B4-lite), 'case-folded' (B2) and 'despaced' (OS-1) are listed
  // alongside but are not HF models either — a bundled JSON lexicon and two
  // relabelings of the existing models' own output, zero download cost — so
  // none of them breaks "free".
  it('enabling art. 9-10 by default adds no new model source (A12 is free)', () => {
    expect(requiredSources(defaultEnabledEntities()).sort())
      .toEqual(['case-folded', 'despaced', 'lexicon', 'multilang-fp32', 'polish-fp16', 'regex']);
  });
